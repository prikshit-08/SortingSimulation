...
void display_text()
{
	...
	if (sorting == 0)	// if not sorting display menu
	{
		...
		bitmap_output(10, 515, "Press r to GENERATE NEW ARRAY",GLUT_BITMAP_9_BY_15);
		bitmap_output(10, 495, "Esc to QUIT",GLUT_BITMAP_9_BY_15);
		bitmap_output(10, 475, "Selected sort: ",GLUT_BITMAP_9_BY_15);
		bitmap_output(150, 475, *(sort_string+sort_count1),GLUT_BITMAP_9_BY_15);
	}
...
void display()
{
	...
	glClear(GL_COLOR_BUFFER_BIT);
		...
		if((swapflag ==1 || sorting==1)&& sort_count1 ==1)
		{
			glColor3f(0,0,1);
			glBegin(GL_POLYGON);
	...
	while(notsorted() && sorting)
	{
		if(dirflag==0)
		{
			while(j<MAX-1)
			{
				...
				j++;
				if(j==MAX-1) {count1++; j=MAX-count1;	dirflag=1-dirflag;}
...
void keyboard (unsigned char key, int x, int y)
{
	...
	if (k==1 && sorting!=1)
	{
		switch (key)
		{
			...
			case 'c' :	sort_count1=(sort_count1+1)%SORT_NO;	break;
		}
	}
	if(k==1 && sorting==1)
		if(key=='p')	sorting=0;
